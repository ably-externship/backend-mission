...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            login_user = AuthUser.objects.filter(username=username)
            print('login_user[0]',login_user[0].is_superuser)
            print('login_user[0].store_number',login_user[0].store_number)
            # print(refresh, '/////////////', access)
            if username == '1234':
                response = redirect('/redstore/items/')
            # if login_user[0].is_superuser ==1:
            #     login_user[0].store_number
            #     response = redirect('')
            # if AuthUser.objects.filter(username=username) and AuthUser.objects.filter(is_superuser=1):
